  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (currentState === 'Sign Up') {
        ...
        const response = await axios.post(`${backendURL}/api/user/login`, { email, password, localIp });
        if (response.data.success) {
    ...
    if (data) {
      try {
        ...
        const { userId, token } = scannedData;
          ...
          const response = await axios.post(`${backendURL}/api/user/qrlogin`, { userId, token, localIp });
